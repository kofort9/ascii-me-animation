...
exports.TerminalRenderer = exports.PhraseCounter = void 0;
const types_1 = require("./types");
const camelotColors_1 = require("./camelotColors");
...
class PhraseCounter {
    calculate(bpm, progressMs, timestamp, timeSignature, isPlaying = true) {
...
exports.PhraseCounter = PhraseCounter;
// Helper to center text in a given width
function center(text, width) {
    const visibleWidth = (0, string_width_1.default)((0, strip_ansi_1.default)(text));
...
function buildRippleState(frameWidth, frameHeight, isActive) {
...
function isInRipple(row, col, ripple) {
    if (!ripple.active)
        return false;
...
function colorizeFrameChar(ch, row, col, ripple, useRippleGlyph = false) {
...
function buildRippleBorderLine(width, rowIndex, ripple, isBottom = false) {
...
function buildRippleTitleLine(title, width, rowIndex, ripple) {
...
function getScanSettings(beatsRemaining) {
    if (beatsRemaining <= 8) {
        return {
            speedMs: 0,
...
function buildStatusBar(width, showWarning, ripple, rowIndex) {
...
class TerminalRenderer {
    ...
    renderTrainBoard(currentTrack, recommendations, phraseInfo, showExitWarning = false, debugMessage, logs = [], selectedCategory = 'ALL', scrollOffset = 0, showHelp = false, notices = [], totalTracksInLibrary = 0) {
        ...
        if (!currentTrack) {
            ...
            if (currentTrack.camelot_key) {
                const hex = (0, camelotColors_1.getCamelotColor)(currentTrack.camelot_key);
            ...
            if (phraseInfo) {
                ...
                if (phraseInfo.beatsRemaining > 0) {
                    ...
                    lines.push(bar, `${labelColor}Beats Rem: ${phraseInfo.beatsRemaining.toFixed(1)} | Time: ${phraseInfo.timeRemainingSeconds.toFixed(1)}s${colors.reset}`, '');
                }
                else if (currentTrack &&
                    currentTrack.audio_features.time_signature &&
                    currentTrack.audio_features.time_signature !== 4) {
                    lines.push(`${colors.yellow}⚠️  Non-4/4 time signature detected (${currentTrack.audio_features.time_signature}/4)${colors.reset}`, `${colors.dim}Phrase counter disabled for non-4/4 tracks${colors.reset}`, '');
                }
            }
        }
        // Render Tabs
        const categories = ['ALL', ...Object.values(types_1.ShiftType)];
        ...
        lines.push(`${colors.bright}${colors.spotifyGreen}⚡ Recommendations${colors.reset} ${colors.dim}(Current: ${currentBpm} BPM)${colors.reset}`);
        lines.push(tabs);
        ...
        if (recommendations.length === 0) {
            if (totalTracksInLibrary === 0) {
                ...
                displayLines.push(`${colors.dim}Tip: export your liked songs as Liked_Songs.csv (Exportify), drop it in this project, and run \`npm run refresh:library\` or press r.${colors.reset}`);
            }
            else {
            ...
            const typeOrder = Object.values(types_1.ShiftType);
            const typesToRender = selectedCategory === 'ALL' ? typeOrder : [selectedCategory];
            for (const type of typesToRender) {
                const tracks = grouped[type];
                if (tracks && tracks.length > 0) {
                    let typeColor = colors.white;
                    if (type === types_1.ShiftType.SMOOTH)
                        typeColor = colors.spotifyGreen;
                    if (type === types_1.ShiftType.ENERGY_UP)
                        typeColor = colors.yellow;
                    if (type === types_1.ShiftType.MOOD_SWITCH)
                        typeColor = colors.magenta;
                    if (type === types_1.ShiftType.RHYTHMIC_BREAKER)
                    ...
                    tracks.forEach(track => {
                        const hex = (0, camelotColors_1.getCamelotColor)(track.camelot_key);
                        const ansiColor = hexToAnsi(hex);
                        ...
                        if (currentTrack && currentTrack.audio_features?.tempo) {
                            const bpmDiff = ((track.bpm - currentTrack.audio_features.tempo) /
                        ...
                        const visualWidth = (0, string_width_1.default)((0, strip_ansi_1.default)(fullLine));
                        if (visualWidth > effectiveWidth) {
                            ...
                            const prefixWidth = (0, string_width_1.default)((0, strip_ansi_1.default)(prefix));
                            const suffixWidth = (0, string_width_1.default)((0, strip_ansi_1.default)(suffix));
                            ...
                            if (availableForTrackInfo > 10) {
                                ...
                                while ((0, string_width_1.default)(truncated) > availableForTrackInfo && truncated.length > 0) {
